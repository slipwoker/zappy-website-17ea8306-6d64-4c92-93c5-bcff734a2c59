document.addEventListener('DOMContentLoaded', function () {
  ...
  if (animatableElements.length > 0) {
    ...
    const injectAnimationStyles = function () {
      if (document.getElementById('scroll-animation-styles')) return;

      const style = document.createElement('style');
      style.id = 'scroll-animation-styles';
      style.textContent = [
        '.fade-in-up, .animate, .fade-in, .scroll-animate, [data-animate],',
        '.section-title, .feature-card, .service-card, .pricing-card,',
        '.testimonial-card, .team-card, .hero-content, .about-content, .cta-content {',
        '  opacity: 0;',
        '  transform: translateY(30px);',
        '  transition: opacity 0.6s ease-out, transform 0.6s ease-out;',
        '  will-change: opacity, transform;',
        '}',
        '.fade-in-up.is-visible, .animate.is-visible, .fade-in.is-visible,',
        '.scroll-animate.is-visible, [data-animate].is-visible,',
        '.section-title.is-visible, .feature-card.is-visible, .service-card.is-visible,',
        '.pricing-card.is-visible, .testimonial-card.is-visible, .team-card.is-visible,',
        '.hero-content.is-visible, .about-content.is-visible, .cta-content.is-visible {',
        '  opacity: 1;',
    ...
    animatableElements.forEach(function (el, index) {
      const delay = el.dataset.animateDelay || (index % 5) * 100;
    ...
    if ('IntersectionObserver' in window) {
      ...
      const checkVisibility = function () {
        ...
        animatableElements.forEach(function (el) {
          ...
          if (visiblePoint < scrollTop + windowHeight - 60) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "פילאטיס עם כוכי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '17ea8306-6d64-4c92-93c5-bcff734a2c59',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '17ea8306-6d64-4c92-93c5-bcff734a2c59';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '17ea8306-6d64-4c92-93c5-bcff734a2c59';
